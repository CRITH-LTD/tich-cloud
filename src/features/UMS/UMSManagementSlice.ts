...
export const fetchUMSById = createAsyncThunk(
  'ums/fetchById',
  async (id: string) => {
    const response = await api.get<UMS>(`/ums/${id}`);
    return response.data;
  }
);
