...
import { CreateFacultyDto, Faculty } from '../../../../../../types/academicUnits.types';
import { generateCode } from '../../../../../../utils';

interface FacultyFormModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSubmit: (data: CreateFacultyDto) => Promise<void>;
    ...
    useEffect(() => {
        if (faculty) {
            ...
            if (faculty && faculty.code) {
                const upperCode = faculty.code.toUpperCase();
                let matched = false;
                for (let len = 2; len <= 4; len++) {
                    const gen = generateCode(faculty.name, len);
                    if (gen === upperCode) {
                ...
                setAutoMode('2');
                setFormData((prev) => ({ ...prev, code: generateCode(faculty.name, 2) }));
            }
        } else {
            setFormData({
                name: '',
