...
import { CreateSchoolDto, School, Faculty } from '../../../../../../types/academicUnits.types';
    ...
    useEffect(() => {
        if (school) {
            ...
            if (school && school.code) {
                const upperCode = school.code.toUpperCase();
                let matched = false;
                for (let len = 2; len <= 4; len++) {
                    const gen = generateCode(school.name, len);
                    if (gen === upperCode) {
                ...
                setAutoMode('2');
                setFormData((prev) => ({ ...prev, code: generateCode(school.name, 2) }));
            }
        } else {
            setFormData({
                name: '',
